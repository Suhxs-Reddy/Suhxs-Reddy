...
Standard YOLO is a phenomenal general-purpose detector, but in production it is inherently narrow — it treats every input frame in a vacuum, processing a clear sunlit highway identically to a rain-slicked 320×240 feed at 3am. In tropical conditions like Singapore, where weather and lighting flip instantly, this context-blindness actively hurts performance. When downpours or night glare suppress pixel-level features, static weights have no mechanism to adapt feature extraction to environmental noise.

This is where **[FiLM](https://arxiv.org/abs/1709.07871)** steps in. Rather than forcing the backbone to infer context from noisy pixels alone, a lightweight **ContextEncoder** (MLP) ingests external metadata — live NEA weather codes, PM2.5 air quality index, hour (sin/cos encoding), image resolution, and a learned 16-dim per-camera embedding that captures viewpoint, road geometry, and typical scene composition for each of the 90 cameras. The camera embeddings are grounded in a hand-built road network: a NetworkX directed graph encoding all 90 LTA cameras as nodes with road name, direction labels, lane anchors, and junction flags — derived by OCR-reading LTA's own text overlays from actual camera footage, with PCA on lat/lon to correctly orient roads like PIE and AYE that confuse naive N/S sorting. The ContextEncoder converts this metadata into scaling (γ) and shifting (β) vectors. FiLM layers apply the affine transform `feature = γ ⊙ feature + β` directly onto intermediate feature maps at P3, P4, and P5 of YOLOv11's backbone — acting as a real-time environmental equaliser that highlights suppressed object signals in degraded conditions.

To preserve YOLO's pretrained intelligence, CATI uses learned adaptive gating: `α · FiLM(feature) + (1−α) · feature`. At initialisation γ=1, β=0, α=0 — the model is identical to vanilla YOLO on day one. As training progresses it learns to activate modulation only where environmental context demonstrably reduces detection loss. Because FiLM operates channel-wise on intermediate tensors rather than adding convolutional layers, the entire conditioning engine adds just **130K parameters on a 9.4M backbone — 1.4% overhead** with no impact on inference latency.
